...
#
# Usage: 
#   sudo ./reproduce-mount-issue.sh [VOLUME_ID] [MODE]
#
# Modes:
...
echo "3. Creating broken symlink at: $TARGET_FILE"
ln -s "/nonexistent/path/file.$$" "$TARGET_FILE"

# Try to mount (will fail but may leave busy state)
echo "4. Attempting mount to create busy state..."
mount --bind "$FAKE_DEVICE" "$TARGET_FILE" 2>&1 | head -1
