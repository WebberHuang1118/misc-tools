...
# Usage: ./create_bi_bbi_cycle.sh [OPTIONS] [KUBECONFIG_PATH]
# Example: ./create_bi_bbi_cycle.sh --bi-name my-image --backing-image-url https://example.com/image.img /home/ubuntu/.kube/arm.yaml
...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --bi-name NAME              Name for the BackingImage (default: bi-noble)"
    echo "  --backing-image-url URL     URL for the backing image (default: Ubuntu Noble ARM64 cloud image)"
    ...
    echo "  BI_NAME                    BackingImage name (overridden by --bi-name)"
    echo "  BACKING_IMAGE_URL          Backing image URL (overridden by --backing-image-url)"
    echo ""
    echo "Examples:"
    echo "  $0"
    echo "  $0 --bi-name my-image /home/ubuntu/.kube/arm.yaml"
    echo "  $0 --backing-image-url https://example.com/image.img"
    echo "  BI_NAME=custom-image BACKING_IMAGE_URL=https://example.com/image.img $0"
    echo ""
    echo "Default Values:"
    echo "  BI_NAME: bi-noble"
    echo "  BACKING_IMAGE_URL: https://cloud-images.ubuntu.com/noble/20250805/noble-server-cloudimg-arm64.img"
    echo ""
    echo "If no KUBECONFIG is specified, kubectl will use its default configuration."
}

# Default values
DEFAULT_BI_NAME="bi-noble"
DEFAULT_BACKING_IMAGE_URL="https://cloud-images.ubuntu.com/noble/20250805/noble-server-cloudimg-arm64.img"

# Initialize configuration with defaults or environment variables
BI_NAME="${BI_NAME:-$DEFAULT_BI_NAME}"
BACKING_IMAGE_URL="${BACKING_IMAGE_URL:-$DEFAULT_BACKING_IMAGE_URL}"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --bi-name)
            if [ -z "$2" ]; then
                echo "Error: --bi-name requires a value"
                exit 1
            fi
            BI_NAME="$2"
            shift 2
        ...
        --backing-image-url)
            if [ -z "$2" ]; then
                echo "Error: --backing-image-url requires a value"
                exit 1
            fi
            BACKING_IMAGE_URL="$2"
            shift 2
